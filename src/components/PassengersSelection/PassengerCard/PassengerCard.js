...
function PassengerCard({
    ...
    if (thisPassenger[0].seatDep || thisPassenger[0].seatArr) {
      ...
      const seatInfoMaker = (direction, seatData) => (
        <div>
          ...
          <span>
            {` вагон: ${
              allCoaches?.filter((el) => el.coachId === seatData[0])[0]?.name
            }
                  , место: ${seatData[1]}`}
          </span>
        </div>
  ...
  const clickOnRemoveCardHandler = () => {
    if (form.getFieldValue(fieldNames.seatArr)) {
      dispatchMaker(form.getFieldValue(fieldNames.seatArr), directions.arrival);
    }
  ...
  const fieldChangeHandler = (value) => {
    ...
    if (
      ...
      const ageChosen =
        dayjs().diff(form.getFieldValue(fieldNames.dateOfBirth), "days") >= 1;

      if (ageChosen) {
        const pasType = form.getFieldValue(fieldNames.passengerType);
        if (ageYears < 10 && pasType === passengerTypes.adults) {
          setLowAgeError(true);
          form.setFieldValue(fieldNames.passengerType, passengerTypes.children);
          seatsFilter(0.5);
        }
      ...
      {expanded && (
        <div className={styles.passengerCard}>
          ...
          >
            ...
            {((!thisPassenger[0] && !disabledForwardBtn) || thisPassenger[0]) &&
              button}
          </Form>
        </div>
      )}
